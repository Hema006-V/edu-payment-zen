...
import { STUDENTS, CLASSES, SECTIONS, inr } from "@/lib/mock-data";
...
function StudentsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] animate-fade-in">
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button onClick={() => toast.success("Add student flow (prototype)")}>
          ...
          <Card key={s.id} className="glass border-0 transition hover:-translate-y-0.5">
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                <Avatar className="h-12 w-12"><AvatarFallback className="bg-gradient-to-br from-primary/30 to-accent/60 text-primary-foreground">{s.name.split(" ").map(x=>x[0]).join("").slice(0,2)}</AvatarFallback></Avatar>
      ...
      {filtered.length === 0 && (
        <div className="glass rounded-2xl p-12 text-center text-muted-foreground">No students match the current filters.</div>
      )}
    </div>
  );
}
